...
useEffect(() => {
  ...
  async function sendNewPostReq(event) {
    ...
    try {
    const responseTwo = await fetch("https://dummyjson.com/posts/add", {
      method: "POST", 
      headers: {
    ...
    const translatedDataTwo = await response.json();
    // console.log(translatedDataTwo)
    setAllPosts([...allPosts, translatedDataTwo]);
    } catch (error) {
      console.log(error)
    }
  }
      ...
      <div>
        ...
        */}
        <form onSubmit={sendNewPostReq}>
          <label htmlFor='title'>Enter New Blog Post Title:</label>
          <br/>
          <input
          name='title'
          type='text'
        ...
        <label htmlFor='body'>Enter New Blog Here:</label>
        <br/>
        <input
        name='body'
        type='text'
        placeholder='New Blog Body Goes Here'
        value={newPostBody}
        onChange={(event) => {
